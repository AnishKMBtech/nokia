document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchScrewData() {
        try {
            ...
            // --- Automatic PDF report generation for every completed cycle ---
            // Track which cycles have already had reports generated
            if (!window.generatedCycleReports) window.generatedCycleReports = new Set();
            // For each complete cycle, generate report if not already done
            cycles.forEach((cycleData, idx) => {
                if (cycleData.length === totalScrews && !window.generatedCycleReports.has(idx)) {
                    // Temporarily set screwDataSet for report generation
                    const prevScrewDataSet = [...screwDataSet];
                    screwDataSet = cycleData.map((screw, i) => ({
                        ...screw,
                        displayId: i + 1,
                        originalId: screw.id,
                        cycleIndex: idx
                    ...
                    generateReport();
                    screwDataSet = prevScrewDataSet; // Restore
                    window.generatedCycleReports.add(idx);
                }
            });
    ...
    function generateReport() {
        ...
        // --- Removed HTML report window logic: do not open new tab/window ---
        ...
        // --- Removed HTML report generation and window/tab logic ---
