document.addEventListener('DOMContentLoaded', () => {
    ...
    // Fetch screw data from the server
    async function fetchScrewData() {
        try {
            const response = await fetch('/api/screws'); // Endpoint on our Node server
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            screwDataSet = await response.json();
            console.log('Screw data fetched successfully:', screwDataSet);
            return true; // Indicate success
        } catch (error) {
            console.error("Failed to fetch screw data:", error);
            alert("Error fetching screw data from the server. Please ensure the server is running and the database is accessible.");
            screwDataSet = []; // Ensure it's empty on error
    ...
    async function startSimulation() { // Make async to await fetch
        console.log('Simulation Started');
        isRunning = true;
        resetDashboard(); // Reset first

        // Fetch data before starting the process
        const dataFetched = await fetchScrewData();
        if (!dataFetched) {
            stopSimulation('Error fetching data'); // Stop if data fetch failed
            return;
        }

        // Check if data was actually loaded
        if (!screwDataSet || screwDataSet.length === 0) {
            console.error("No screw data available after fetch.");
            alert("No screw data loaded. Cannot start simulation.");
            stopSimulation('Error');
            return;
        }
